...
read_census <- function(stat){
  ...
  file_name <- 
    str_c("republic-of-ireland", stat, "sa", "2016", sep = "_") |> 
    str_c(".RDS")
...
#| cache: false
dioceses <-
  read_sf(here("geospatial-data", "coi-dioceses-as-small-area-conglomerations.geojson"))
...
stats <-
  map(stats,
      \(stat){
        ...
        stat <- 
          mutate(stat,
                 area_code = str_remove(sa_2015_code, "SA2017_")) |> 
                   select(-sa_2015_code)
...
iwalk(dioceses_tables, 
    \(x, x_name){
      file_name <- 
        str_c("roi", "dioceses", x_name, "2016", sep = "_") |> 
        str_c(".csv")
